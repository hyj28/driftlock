...
def test_semantic_paraphrase_needs_no_shared_content_word(tmp_path: Path) -> None:
    ...
    tool = AgenticRetrievalTool.from_workspace(
        workspace,
        library,
        LiteralEmbedder({activation: (1.0, 0.0), query: (1.0, 0.0)}),
    )
...
def test_pinned_embedder_separates_live_situation_from_alp_instruction(
    ...
    if importlib.util.find_spec("st_embedder") is None:
        pytest.skip("pinned st_embedder.py is unavailable")
    try:
        module = importlib.import_module("st_embedder")
        embed = module.embed
    except (AttributeError, ImportError, OSError, RuntimeError) as error:
        pytest.skip(f"pinned st_embedder.py is unavailable: {error}")
...
def test_per_call_result_cap_and_task_budget_are_explicit(tmp_path: Path) -> None:
    ...
    assert exhausted.status is AgenticRetrievalStatus.TASK_BUDGET_EXHAUSTED
...
def test_per_call_character_cap_is_explicit(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    source = "oversized matching document"
