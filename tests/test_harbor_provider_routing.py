...
async def test_native_adapter_emits_explicit_cache_control_when_configured(
    tmp_path: Path,
    harbor_agent_modules: tuple[Any, Any],
) -> None:
    _, native_agent = harbor_agent_modules
    agent = native_agent.LHTBNativeDriftlockAgent(
        ...
        driftlock_explicit_prompt_cache_control=True,
    )
...
async def test_native_adapter_does_not_infer_cache_control_from_model_name(
    tmp_path: Path,
    harbor_agent_modules: tuple[Any, Any],
) -> None:
    _, native_agent = harbor_agent_modules
    agent = native_agent.LHTBNativeDriftlockAgent(
        logs_dir=tmp_path,
        model_name="openai-compatible/claude-proxy",
        llm_call_kwargs=_agent_call_kwargs(),
        model_info={
            "max_input_tokens": 128000,
            "max_output_tokens": 8192,
    ...
    assert captured["prompt"] == "abcdefgh"
