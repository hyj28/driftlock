...
@pytest.mark.asyncio
async def test_validation_summary_counts_unobservable_process_baselines(
    tmp_path: Path,
) -> None:
    root = _lhtb_tree(tmp_path)
    plan = plan_skill_validation(_candidate_file(tmp_path), root)
    ...
    assert attempt["audit"]["unobservable_process_baseline_count"] == 1
    assert report["validation"]["summary"]["unobservable_process_baseline_count"] == 1
