...
async def test_malformed_cache_usage_matrix_is_typed_and_never_raw(
    ...
    if cache:
        ...
        assert outcome.error.startswith("Malformed provider cache report: provider ")
        assert outcome.prompt_cache is None
    else:
...
def test_summary_excludes_unobservable_steps_from_rates_and_records_cap() -> None:
    hit = prompt_cache_report(prompt_tokens=100, cached_tokens=75, invalidation=None)
    miss = prompt_cache_report(prompt_tokens=50, cached_tokens=0, invalidation=None)
    blind = prompt_cache_report(
        prompt_tokens=None, cached_tokens=None, invalidation=None
    )

    mixed = summarize_prompt_cache((hit, miss, blind))
    unobservable = summarize_prompt_cache((blind,))
    capped = summarize_prompt_cache(
        tuple(blind for _ in range(DEFAULT_MAX_CACHE_SUMMARY_REPORTS + 1))
    )
    late_blind = summarize_prompt_cache(
        (*tuple(hit for _ in range(DEFAULT_MAX_CACHE_SUMMARY_REPORTS)), blind)
    )
    ...
    assert capped.summarized_report_count == 256
    assert capped.omitted_report_count == 1
    assert capped.to_dict()["report_limit_reached"] is True
    assert late_blind.observability is PromptCacheObservability.PARTIAL


async def test_compaction_records_observed_reprime_cost() -> None:
    history = [
        {"role": "user", "content": "old context " * 300},
        {"role": "assistant", "content": "continue"},
    ]
    state = AgentConversationCodec().encode(history, steps=2)
    provider = ScriptedProvider([AgentCompletion(prompt_tokens=400, cached_tokens=25)])
    ...
    assert outcome.prompt_cache == PromptCacheReport(
        ...
        prompt_tokens=400,
        cached_tokens=25,
        invalidation=PromptCacheInvalidation.COMPACTION,
        reprimed_input_tokens=375,
    )


async def test_rollback_records_reprime_cost_and_preserves_tool_pairing() -> None:
    ...
    assert outcome.prompt_cache == PromptCacheReport(
        ...
        prompt_tokens=300,
        cached_tokens=0,
        invalidation=PromptCacheInvalidation.ROLLBACK,
        reprimed_input_tokens=300,
    )
...
async def test_runner_rollback_restores_prefix_and_records_next_reprime(
    tmp_path: Path,
) -> None:
    ...
    assert result.steps[-1].outcome.prompt_cache == PromptCacheReport(
        ...
        cached_tokens=0,
        invalidation=PromptCacheInvalidation.ROLLBACK,
        reprimed_input_tokens=125,
    )
    assert result.prompt_cache_summary is not None
    assert result.prompt_cache_summary.invalidation_counts["rollback"] == 1
    assert result.prompt_cache_summary.reprimed_input_tokens["rollback"] == 125
...
def test_all_prompt_cache_enum_values_are_unique_and_all_directions_render() -> None:
    ...
    observed = summarize_prompt_cache(
        (
            prompt_cache_report(
                prompt_tokens=10,
                cached_tokens=1,
                invalidation=PromptCacheInvalidation.COMPACTION_AND_ROLLBACK,
            ),
        )
    )
    assert observed.observability is PromptCacheObservability.OBSERVED
    assert observed.invalidation_counts == {
        "compaction": 0,
        "rollback": 0,
        "compaction_and_rollback": 1,
    }
    assert observed.reprimed_input_tokens["compaction_and_rollback"] == 9
